...
! $Id: commLine.f,v 1.1 2011-04-01 14:38:02 gelpi Exp $
! $Revision: 1.1 $
!
MODULE commLine
...
function openFn (opts, descr, unit, ext) result(unitf)
 ...
 if (unitf.gt.0)  then
    open (unitf, file=trim(opts(unitf)%filename)//trim(ext), status=opts(unitf)%status, &
       form=opts(unitf)%fmt, iostat=err)
    if (err.ne.0) then
